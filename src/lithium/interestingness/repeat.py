...
2. Unstable testcases.
   ...
   Varying a number in the test (using REPEATNUM) may allow other parts of the testcase
   to be removed (Lithium), or may allow different versions of the shell to crash
   (autoBisect).
   ...
   On the command line: (SpiderMonkey-specific example)
     python -m lithium repeat 20 crashes --timeout=9 \
       ./js --fuzzing-safe -e "n=REPEATNUM;" testcase.js
"""
